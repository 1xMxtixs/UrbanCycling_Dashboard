...
export function ListClientes() {
  ...
  const fetchClientes = async () => {
    ...
    try {
      ...
      const naturales: ClienteNatural[] = dbClientes
        ...
        .map((c) => {
          const nombreComp = [c.primerNombre, c.segundoNombre]
            .filter(Boolean)
            .join(" ")
          const apellidoComp = [c.apellidoPaterno, c.apellidoMaterno]
            .filter(Boolean)
            .join(" ")
  ...
  }, [clienteIdParam, rawClientes, isCurrentParameterDismissed])
  ...
  const selectedCliente = rawClientes.find(
    (c) => c.idCliente === selectedClienteId
  )
    ...
    <div className="animate-in space-y-6 duration-300 fade-in">
      ...
      <Dialog open={openDetailsModal} onOpenChange={handleDetailsOpenChange}>
        <DialogContent
          ...
          className="flex max-h-[90vh] flex-col overflow-hidden rounded-2xl border border-border/80 bg-card p-0 text-card-foreground shadow-2xl sm:max-w-4xl lg:max-w-5xl"
        >
          {selectedCliente &&
            (() => {
              ...
              const initials = isNatural
                ...
                : (
                    selectedCliente.razonSocial?.slice(0, 2) || "PJ"
                  ).toUpperCase()
              ...
              return (
                <>
                  <DialogHeader className="sr-only">
                    ...
                    <DialogDescription>
                      Información detallada, contactos, direcciones y órdenes de
                      trabajo del cliente {fullName}.
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <div className="border-b border-border/60 bg-muted/30 p-6">
                    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1">
                          <div className="flex flex-wrap items-center gap-2.5">
                            ...
                            <StatusBadge
                              status={
                                selectedCliente.estado.toLowerCase() ===
                                "activo"
                                  ? "success"
                                  : "danger"
                              }
                      ...
                      <Button
                        ...
                        className="cursor-pointer gap-1.5 self-start rounded-xl text-xs font-semibold sm:self-center"
                      >
                        <History className="h-3.5 w-3.5 text-primary" /> Ver
                        Historial Completo
                      </Button>
                    </div>
                  </div>
                  ...
                  {/* Content Section */}
                  <div className="grid max-h-[60vh] flex-1 gap-6 overflow-y-auto p-6 md:grid-cols-5">
                    {/* Left Column */}
                    <div className="space-y-4 md:col-span-3">
                      {!isNatural && (
                        <div className="space-y-3 rounded-xl border border-border/60 bg-muted/20 p-4">
                          <h4 className="flex items-center gap-1.5 border-b border-border/40 pb-2 text-[11px] font-bold tracking-wider text-foreground text-muted-foreground uppercase">
                          ...
                          <div className="grid gap-3 sm:grid-cols-2">
                            ...
                            <DataField
                              label="Contacto"
                              value={
                                selectedCliente.nombreContacto ||
                                "No especificado"
                              }
                            />
                          </div>
                      ...
                      <div className="space-y-4 rounded-xl border border-border/60 bg-muted/20 p-4">
                        ...
                        <div className="space-y-3 text-xs">
                          <div>
                            ...
                            {selectedCliente.telefonos &&
                            selectedCliente.telefonos.length > 0 ? (
                              <div className="space-y-1.5">
                                {selectedCliente.telefonos.map((t) => (
                                  ...
                                  >
                                    ...
                                    {t.descripcion && (
                                      <span className="rounded-md border border-border/60 bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">
                                        {t.descripcion}
                                      </span>
                                    )}
                                  </div>
                          ...
                          <div>
                            ...
                            {selectedCliente.correos &&
                            selectedCliente.correos.length > 0 ? (
                              <div className="space-y-1.5">
                                ...
                                {selectedCliente.direcciones.map((d) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <div>
                                        <span className="block leading-relaxed font-semibold">
                                          {formattedAddress}
                                        </span>
                    ...
                    <div className="space-y-4 md:col-span-2">
                      <div className="flex items-center justify-between">
                        <h4 className="flex items-center gap-1.5 text-[11px] font-bold tracking-wider text-foreground text-muted-foreground uppercase">
                          <ClipboardList className="h-4 w-4 text-primary" />
                          Historial de Órdenes (
                          {selectedCliente.ordenesDeTrabajo?.length || 0})
                        </h4>
                      </div>

                      {selectedCliente.ordenesDeTrabajo &&
                      selectedCliente.ordenesDeTrabajo.length > 0 ? (
                        <div className="max-h-[35vh] space-y-2.5 overflow-y-auto pr-1">
                          {selectedCliente.ordenesDeTrabajo.map((order) => {
                            const isCompleted = [
                              "listo para entregar",
                              "entregado",
                            ].includes(order.estadoOrden.toLowerCase())
                            const isWarning = [
                              "en espera",
                              "en curso",
                            ].includes(order.estadoOrden.toLowerCase())
                        ...
                        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/80 bg-muted/20 p-6 text-center">
                          <ClipboardList className="mb-1 h-6 w-6 stroke-[1.5] text-muted-foreground" />
